...
8) 再点击「新增浏览器」一次，在「名称」和「指令」栏分别输入`国外视频下载`和`"C:\AnnieDownloader\yt-dlp\yt-dlp.exe" --output "替换这里" -f "bestvideo[height<=1080]+bestaudio/best[height<=1080]" --output "E:/Downloads/Video/%(title)s.%(ext)s" --merge-output-format mp4` ，将 替换这里 替换为第6步获取到的路径，点击「添加」。
